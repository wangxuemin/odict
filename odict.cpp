...
template<typename SIGN_TYPE,typename CUSTOM_TYPE>struct OdictSNode{
    SIGN_TYPE sign1; //signature1
    SIGN_TYPE sign2; //signature2
    typename CUSTOM_TYPE::RAW cuint1; //custom data1
    typename CUSTOM_TYPE::RAW cuint2; //custom data2
};
// typedef OdictSNode<unsigned int,Custom_unsigned_int> sodict_snode_t;
// typedef OdictSNode<unsigned long long,Custom_unsigned_int> sodict128_snode_t;
...
template<typename SIGN_TYPE,typename CUSTOM_TYPE>struct OdictBHash{
    OdictBNode<SIGN_TYPE,CUSTOM_TYPE> *pnode; //hash table..
};

//internal use..
template<typename SIGN_TYPE,typename CUSTOM_TYPE>struct OdictBNodeBlock{
    OdictBNode<SIGN_TYPE,CUSTOM_TYPE> nodes[ODB_NODE_BLOCK_NUM]; //block of nodes.
    OdictBNodeBlock<SIGN_TYPE,CUSTOM_TYPE>*next; //next block.
};

//internal use..
template<typename SIGN_TYPE,typename CUSTOM_TYPE>struct OdictBuild{
    int hash_num; //hash node.
    int node_num; //how many nodes.
    int cur_code; //current operation count(by caller)
    OdictBHash<SIGN_TYPE,CUSTOM_TYPE> *hash; //hash table
    OdictBNodeBlock<SIGN_TYPE,CUSTOM_TYPE> *nblock; //first block
    OdictBNodeBlock<SIGN_TYPE,CUSTOM_TYPE> *cur_nblock; //current block
    int node_avail; //avail node number in current block
    OdictBNode<SIGN_TYPE,CUSTOM_TYPE> * cur_node; //current node for use.
...
template<typename SIGN_TYPE,typename CUSTOM_TYPE>struct OdictSearch{
    ...
    OdictSNode<SIGN_TYPE,CUSTOM_TYPE> *node; //nodes.
    typedef SIGN_TYPE SIGN;
    typedef CUSTOM_TYPE CUSTOM;
};

struct _sodict_build_t:public OdictBuild<unsigned int,Custom_unsigned_int>{
};
struct _sodict_search_t:public OdictSearch<unsigned int,Custom_unsigned_int>{
};
struct _sodict128_build_t:public OdictBuild<unsigned long long,Custom_unsigned_int>{
};
struct _sodict128_search_t:public OdictSearch<unsigned long long,Custom_unsigned_int>{
};
...
/*path可以为空，filename不能为空,0不存在，1存在*/
...
static ssize_t odbi_write(int fno, void *data, size_t len)
{
    ssize_t rsize = 0;
    ssize_t lsize = 0;
    size_t readed = 0;
    ...
    while(readed < len) {
        rsize = ((len - readed) <ODB_IO_BUFFSIZE) ? (len - readed) : ODB_IO_BUFFSIZE;
        lsize = ul_write(fno, (char*)data + readed, rsize);
        if (lsize < 0 || lsize != rsize) {
            return -1;
        }
        readed += lsize;
    }
    return (readed == len) ? (ssize_t)readed : -1;
}
    ...
    static OdictBuild<SIGN_TYPE,CUSTOM_TYPE> *_odb_creat(int hash_num)
{
    ...
    if (hash_num <= 0 || hash_num > MAX_HASH_NUM) {
        ul_writelog(UL_LOG_WARNING, "[ODICT WARNING] hash number is wrong, will be set to default value[%d].", 
					ODB_DEFAULT_HASH_NUM);
    ...
    static OdictBuild<SIGN_TYPE,CUSTOM_TYPE> *_odb_load(char *path, char *filename, int hash_num)
{
	...
	if (hash_num > MAX_HASH_NUM) {
        ul_writelog(UL_LOG_WARNING, "[ODICT WARNING] hash number is to large, will be set to default value[%d].", 
					ODB_DEFAULT_HASH_NUM);
		hash_num = ODB_DEFAULT_HASH_NUM;
	}
    ...
    static OdictSearch<SIGN_TYPE,CUSTOM_TYPE> *_odb_load_search(char *path, char *filename)
{
	...
	ssize_t rd_len = hash_num*sizeof(odict_offset_t);
    odbi_read(fno, pds->hash, (odict_offset_t)rd_len);
    odbi_read(fno, pds->num, (odict_offset_t)rd_len);  // read the number of nodes in hash entries
    ul_close(fno);
	...
	rd_len = node_num*sizeof(_inl_sodict_snode_t);
    odbi_read(fno, pds->node, (size_t)rd_len);
    ul_close(fno);
    ...
    static int _odb_save(OdictBuild<SIGN_TYPE,CUSTOM_TYPE>* sdb, char *path, char *filename)
{
    ...
    odict_offset_t   *phi;           /* the hash of dict_searcher */
    odict_offset_t   *pnum;          /* number of nodes in hash entries */
    ...
    for (i = 0; i < h_num; i++) {
        ...
        while (pnode != NULL) {
            ...
            if (j >= ODB_MID_NODE_NUM) {
                ...
                if (odbi_write(fno, ds, wt_len) != wt_len) {
    ...
    if (odbi_write(fno, phi, wt_len) != wt_len) {
    ...
    wt_len = h_num * sizeof(odict_offset_t);
    if (odbi_write(fno, pnum, wt_len) != wt_len) {
    ...
    static int _odb_save_search(OdictSearch<SIGN_TYPE,CUSTOM_TYPE> *sdb, char *path, char *filename)
{
    ...
    if (odbi_write(fno, sdb->hash, wt_len) != wt_len) {
    ...
    if (odbi_write(fno, sdb->num, wt_len) != wt_len) {
    ...
    if (odbi_write(fno, sdb->node, wt_len) != wt_len) {
    ...
    static int _odb_renew(OdictBuild<SIGN_TYPE,CUSTOM_TYPE> *sdb)
{
    ...
    //    memset(sdb->cur_node, 0, ODB_NODE_BLOCK_NUM*sizeof(_inl_sodict_bnode_t));
    ...
    static int _odb_del(OdictBuild<SIGN_TYPE,CUSTOM_TYPE> *sdb, OdictSNode<SIGN_TYPE,CUSTOM_TYPE> *snode)
{
    ...
    /* If both the signatures are zero, it is invalid */
 ...
 */
template<typename SIGN_TYPE,typename CUSTOM_TYPE>
    static void report_node(OdictSNode<SIGN_TYPE,CUSTOM_TYPE> *node, int * /*stop_now*/)
{
    //使用std::cerr而不是fprintf..
