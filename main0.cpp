/*
 * $Id: main0.cpp,v 1.2 2009/09/17 03:58:04 yufan Exp $
...
/*
	printf("Insert a node into odict dictionary object ... \t");
	snode.sign1 = 25;
	snode.sign2 = 30;
	snode.cuint1 = 28;
	...
	printf("Now report the status of the dictionary object :\n");
	odb_build_report(sdb);
	printf("\n");

	printf("Insert another node into odict dictionary object ... \t");
	snode.sign1 = 26;
	snode.sign2 = 70;
	snode.cuint1 = 27;
	snode.cuint2 = 0xf0ffffff;
	if (ODB_ADD_OK != odb_add(sdb, &snode, 1)) {
		printf("FAIL\n");
		return 2;
	}
	...
	printf("Now reload the dictionary for building test\n");
	sdb = odb_load("./", "test_dict", 0);
*/
	...
	// reload for searching
	printf("Now reload the dictionary for searching test\n");
	ssdb = odb_load_search_mmap("./", "test_dict");
	search_action(ssdb);

	printf("Now destroy the dictionary object!\n");
	odb_destroy_search_mmap(ssdb);
